  ...
  return function (state) {
    ...
    */
    return new Promise(function (resolve, reject) {
      ...
      var columnList = columns.join(', '); // 1. construct the INSERT statement

      var insertStmt = "INSERT INTO ".concat(table, " (").concat(columnList, ")"); // 2. construct the subquery

      var subqueryAlias = 'a';
      var selectValues = rows.map(function (r) {
        var selectColumns = columns.map(function (c) {
          return "".concat(_mysql["default"].escape(r[c]), " as ").concat(c);
        }).join(', ');
        return "SELECT ".concat(selectColumns);
      });
      var subqueryStmt = selectValues.join(' UNION ');
      var selectStmt = "SELECT * FROM (".concat(subqueryStmt, ") as ").concat(subqueryAlias); // 3. construct the ON DUPLICATE KEY statement

      var updateStmt = columns.map(function (c) {
        return "".concat(c, " = ").concat(subqueryAlias, ".").concat(c);
      }).join(', ');
      var onduplicateStmt = "ON DUPLICATE KEY UPDATE ".concat(updateStmt); // combine 1, 2 & 3

      var upsertString = "".concat(insertStmt, " ").concat(selectStmt, " ").concat(onduplicateStmt);
